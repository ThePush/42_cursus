...
void Bureaucrat::signForm(AForm& form) const {
    std::cout << KCYN "Bureaucrat::signForm() [" << this->m_name << "] called: " KNRM;
    try {
        form.beSigned(*this);
        std::cout << KGRN "signed form [" << form.getName() << "] ✓" KNRM << std::endl;
    } catch (std::exception const& e) {
        std::cout << KRED "could not sign form [" << form.getName() << "]"
                  << " because " << e.what() << " ✗" KNRM << std::endl;
    }
}
...
void Bureaucrat::executeForm(AForm const& form) {
    try {
        form.execute(*this);
        std::cout << KGRN "Bureaucrat::executeForm() [" << this->m_name << "]: executed [" << form.getName() << "] ✓" KNRM << std::endl;
    } catch (std::exception const& e) {
        std::cout << KRED "Bureaucrat::executeForm() [" << this->m_name << "]: could not execute [" << form.getName() << "] because " << e.what() << " ✗" KNRM << std::endl;
    }
}
